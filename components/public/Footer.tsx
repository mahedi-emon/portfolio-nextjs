...
import type { About, Contact } from "@/lib/cms/types";
...
export function Footer({ about, contact }: Props) {
  ...
  const handleAnchorClick = useCallback(
    (e: React.MouseEvent, hash: string) => {
      ...
      if (pathname === "/") scrollToElement();
      else router.push(`/${hash.replace("/", "")}`);
    },
    [pathname, router],
  );
    ...
    <footer className="relative w-full bg-[#0B1320] text-white overflow-hidden">
      ...
      <div className="relative mx-auto w-full max-w-[1400px] px-6 sm:px-8 lg:px-12 xl:px-16 pt-16 sm:pt-20 pb-8 sm:pb-10">
        <div className="grid gap-8 sm:gap-12 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-12 sm:mb-16">
            ...
            <div className="flex gap-3">
              {socialLinks.slice(0, 5).map((link, index) => {
                ...
                return (
                  <a
                    key={link.url}
                    className="w-10 h-10 rounded-xl bg-[#0B1320]/60 border border-white/10 flex items-center justify-center text-white/60 hover:bg-[#C77DFF] hover:border-[#C77DFF] hover:text-white hover:-translate-y-1 transition-all duration-300"
                    href={link.url}
                    target="_blank"
                    rel="noreferrer"
          ...
          {/* Quick Links */}
          <nav className="space-y-6 animate-fade-in" style={{ animationDelay: "100ms" }}>
        ...
        <div className="pt-8 border-t border-white/10 animate-fade-in" style={{ animationDelay: "400ms" }}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex gap-6 text-sm text-white/60">
              {legalLinks.map((link) => (
                <a key={link.label} className="hover:text-white hover:-translate-y-0.5 transition-all duration-300" href={link.href}>
                  {link.label}
                </a>
              ))}
            </div>
