...
export function AboutPageClient({ data }: { data: AboutData }) {
  const { about, contact, skills, education, experience, certifications } = data;
            ...
            >
              ...
              <div className="profile-card-inner relative bg-[#0B1320]/80 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-white/[0.06] transition-all duration-500 ease-out">
                ...
                <div className="mt-6 text-center">
                  <h2 className="text-xl font-bold text-white hover:text-[#C77DFF] transition-colors">
                    {about.fullName ?? "Your Name"}
                  </h2>
                  <p className="text-[#C77DFF] font-medium text-shimmer">
                    {about.currentRole ?? "Your Role"}
                  </p>
                </div>
            ...
            {Array.isArray(about.highlights) && about.highlights.length > 0 && (
              <div className="mt-8 grid sm:grid-cols-2 gap-3">
                {about.highlights.map((item: string, index: number) => (
                  <div
                    key={item}
                    className="flex items-start gap-3 p-4 rounded-xl bg-[#0B1320]/80 backdrop-blur-sm border border-white/[0.06] shadow-lg shadow-[#C77DFF]/[0.05] hover:shadow-xl hover:shadow-[#C77DFF]/[0.12] hover:-translate-y-1 hover:border-white/[0.12] transition-all duration-600 ease-out"
                    style={{ animationDelay: `${index * 100}ms` }}
                  >
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#C77DFF]/10 border border-[#C77DFF]/15 text-[#C77DFF] text-sm font-bold transition-all duration-500 ease-out">
                      {index + 1}
                    </div>
            ...
            <div className="animate-fade-in" style={{ animationDelay: "200ms" }}>
              <div className="flex items-center gap-3 mb-6">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#C77DFF]/10 border border-[#C77DFF]/15 transition-all duration-500 ease-out">
                  <Code className="w-5 h-5 text-[#C77DFF]" />
                </div>
                    ...
                    >
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {dateRange && <p className="text-white/50 text-sm mt-0.5">{dateRange}</p>}
                          {item.grade && (
                          ...
                          {item.activities && (
                            <p className="text-white/60 text-sm mt-1">
                              ...
                              <span className="text-[#C9D1D9]">{item.activities}</span>
                            </p>
                          )}
                          {item.description && (
                ...
                <div className="space-y-6">
                  {experience.slice(0, 5).map((item, index) => (
                    ...
                    >
                      ...
                      <div className="p-6 rounded-2xl bg-[#0B1320]/80 backdrop-blur-sm border border-white/[0.06] shadow-lg shadow-[#C77DFF]/[0.05] hover:shadow-xl hover:shadow-[#C77DFF]/[0.12] hover:-translate-y-1 hover:border-white/[0.12] transition-all duration-600 ease-out">
                        <h3 className="text-lg font-semibold text-white hover:text-[#C77DFF] transition-colors">
                          {item.role}
                        </h3>
            ...
            <div className="animate-fade-in" style={{ animationDelay: "500ms" }}>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {certifications.map((item, index) => (
                  <article
                    key={item.id}
                    className="group p-5 rounded-2xl bg-[#0B1320]/80 backdrop-blur-sm border border-white/[0.06] shadow-lg shadow-[#C77DFF]/[0.05] hover:shadow-xl hover:shadow-[#C77DFF]/[0.12] hover:border-white/[0.12] hover:-translate-y-1 transition-all duration-600 ease-out"
                    style={{ animationDelay: `${index * 100}ms` }}
                  >
                    <h3 className="font-semibold text-white group-hover:text-[#C77DFF] transition-colors">
                      {item.certificateTitle}
                    </h3>
                    <p className="text-sm text-[#C77DFF] font-medium">{item.issuer}</p>
                    <p className="text-xs text-white/60 mt-1">{item.issueDate}</p>
                  </article>
                ))}
              </div>
            </div>
