...
const extractLabelFromHost = (host: string): string => {
  if (!host) return "Custom";
  const core = host.split(".")[0] || host;
  return core.charAt(0).toUpperCase() + core.slice(1);
};

export const formatPlatformLabel = (platform: string, fallbackLabel?: string): string => {
  switch (platform) {
...
export const detectSocialPlatform = (url: string): DetectedSocial => {
  ...
  if (host.includes("mastodon")) {
    return { platform: "mastodon", iconKey: "mastodon", label: "Mastodon" };
  }
